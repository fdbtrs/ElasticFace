...
def main(args):
    ...
    margin_softmax = eval("losses.{}".format(cfg.loss))(cfg.s,cfg.m)
    ...
    opt_backbone = torch.optim.SGD(
        params=[{'params': backbone.module.features.parameters()},{'params': backbone.module.tail.parameters(), 'weight_decay': cfg.weight_decay_last}
                ,{'params': backbone.module.feautre_layer.parameters(), 'weight_decay':cfg.weight_decay_last}],
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for step, (img, label, qs_score) in enumerate(train_loader):
            global_step += 1
            features = F.normalize(backbone(img))
            x_grad, loss_v = module_partial_fc.forward_backward(label , qs_score, features, opt_pfc)
