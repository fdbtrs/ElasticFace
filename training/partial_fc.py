...
class PartialFC(Module):
    ...
    def sample(self, total_label):
        ...
        if int(self.sample_rate) != 1:
            ...
            if self.num_sample - positive.size(0) >= 0:
                ...
                #perm= torch.topk(qs_score, k=self.num_sample,dim=0)[0]
                perm[positive] = 2.0
    ...
    def prepare(self, label, qs_score, optimizer):
        with torch.cuda.stream(self.stream):
            total_label = torch.zeros(
                size=[self.batch_size * self.world_size], device=self.device, dtype=torch.long)
            total_qs_score= torch.zeros(
                size=[self.batch_size * self.world_size], device=self.device, dtype=torch.long)
            dist.all_gather(list(total_label.chunk(self.world_size, dim=0)), label)
            dist.all_gather(list(total_qs_score.chunk(self.world_size, dim=0)), qs_score)
            self.sample(total_label)
            optimizer.state.pop(optimizer.param_groups[-1]['params'][0], None)
            optimizer.param_groups[-1]['params'][0] = self.sub_weight
            optimizer.state[self.sub_weight]['momentum_buffer'] = self.sub_weight_mom
            norm_weight = normalize(self.sub_weight)
            return total_label,total_qs_score, norm_weight

    def forward_backward(self, label, qs_score, features, optimizer):
        total_label,total_qs_score, norm_weight = self.prepare(label,qs_score, optimizer)
        ...
        logits = self.margin_softmax(logits, total_label, total_qs_score)
